        ...
        players = {
            {
                ...
                cards = {
                    ...
                    hand = {
                        --{ qty = 1, card = reveal_test_carddef() },
...
--=================
function reveal_test_carddef()
	local card = e.NewCardDef("wizard_magic_mirror", "textures/wizard_magic_mirror")

    card.Name = "Magic Mirror"
    card.DisplayName = "Magic Mirror"
    card.Description = "<sprite name=\"Point\"><color=#3BF2FF><b>Available at level </b></color> 11"
    card.CardType = CardType.Item
    card.CardSubTypes = {wizardType}
    card.Tags = { wizardGalleryCardTag }
    card.Level = 11
	...
	local effReveal2 =  moveTarget(revealPloc).apply(selectLoc(tradeDeckLoc).take(2).reverse())
							

    local ab = createAbility({
        id = "wizard_magic_mirror",
        effect = effReveal2,
        cost = noCost,
        trigger = onPlayTrigger,
        playAllType = noPlayPlayType,
        tags = { draw1Tag, draw2Tag },
